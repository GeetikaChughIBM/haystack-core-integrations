...
def parse_query_with_filters(user_query):
    ...
    remaining_query = re.sub(
        r"\b(for|with|and|show|me|find)\b",
        "",
        remaining_query,
        flags=re.IGNORECASE,
    )
...
def search_products(pipeline, query, filters=None):
    ...
    print("\n" + "-" * 80)
    print(colored(f"Query: {query}", Colors.YELLOW))
...
def main():
    ...
    search_products(
        pipeline,
        "comfortable recovery shoes for jogging",
    )

    search_products(
        pipeline,
        "professional basketball court sneakers",
    )

    search_products(
        pipeline,
        "waterproof trekking shoes for mountain trails",
    )

    search_products(
        pipeline,
        "formal office shoes for business meetings",
    )

    search_products(
        pipeline,
        "boots for snow and freezing temperatures",
    )
    ...
    queries = [
        ...
        "waterproof hiking shoes in stock",

        "formal black office shoes",

        "basketball sneakers for indoor courts",

        "winter boots for snow",
    ]
